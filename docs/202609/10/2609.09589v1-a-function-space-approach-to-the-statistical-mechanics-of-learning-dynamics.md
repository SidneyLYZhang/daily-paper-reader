...
title_zh: 学习动力学的统计力学的函数空间方法
authors: "Yizhou Zhang, Weichen Wu, Lun Du, Zhengjie Miao"
date: 2026-09-09
pdf: "https://arxiv.org/pdf/2609.09589v1"
tags: ["query:ml-theory"]
score: 6.0
evidence: 学习动力学的函数空间统计力学
tldr: "深度网络参数空间高维非线性，却呈现规则宏观学习行为。本文在函数空间建立统计力学描述，把参数组态视为微观实现、函数及动力学算子视为宏观变量。对均方损失，误差动力学由学习算子 M=JJ* 精确控制，并通过涨落积分得到含统计算子 B 的自由能项。该框架表明 M 与 B 谱匹配决定稳定性，低 B 对应低结构曲率与沿平滑数据自适应方向的更快松弛，为理解学习的稳定集体组织提供宏观视角。"
source: arxiv
selection_source: fresh_fetch
motivation: 深度网络参数空间巨大且非线性强，但学习常表现出规则宏观行为，亟需在合适的宏观层面刻画其稳定集体组织。
method: 作者在函数空间构建统计力学框架，将参数组态视为微观实现、函数与动力学算子视为宏观变量，并对局部涨落积分得到含 M 与 B 的自由能项。
result: "对均方损失，误差动力学由 M=JJ* 控制；涨落自由能项在 [M,B]=0 时旋转稳定，偏好 M 大特征值与 B 小特征值配对，ReLU 下低 B 对应低结构曲率。"
conclusion: 结果表明函数空间是研究学习动力学稳定集体组织的自然宏观层面，为理解深度网络规则行为提供统计力学视角。
---

## 摘要
深度神经网络尽管在庞大的参数空间中具有高度非线性动力学，却展现出规则的宏观行为。我们直接在函数空间中建立学习的统计力学描述，将参数构型视为微观实现，而将函数及其动力学算子视为宏观变量。对于均方损失，精确的误差动力学由学习算子 \(M=JJ^\ast\) 支配。将条件随机动力学的动力学玻尔兹曼权重与参数空间态密度（其局部曲率定义统计算子 \(B\)）相结合，并对局部涨落积分，得到 $$ Φ_{\mathrm{fluc}}(M;B)=\frac{σ_ξ^2}{2}\log\det(M^{-1}+B)+\mathrm{const}. $$ 在固定谱下，当 \([M,B]=0\) 时，该项在旋转下平稳；通过将 \(M\) 的大特征值与 \(B\) 的小特征值配对，该项被最小化，并产生一个抵抗旋转失配的局部回复贡献。对于在温和稳定统计条件下的 ReLU 型函数空间，\(B=σ_ξ^2L^\ast\mathcal K L\)，其中 \(L\) 度量粗粒化二阶结构。因此，低 \(B\) 扇区在 \(\mathcal K\) 的有界各向异性范围内对应于低结构曲率，意味着偏好沿光滑、数据自适应方向更快弛豫。这些结果将函数空间确定为研究学习中稳定集体组织的自然宏观层次。
